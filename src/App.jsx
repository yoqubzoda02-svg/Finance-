...
function weekKey(dateStr) {
  // Returns ISO date string of the Monday for the week containing dateStr
...
// Groups salary entries and penalties into weeks (Mon–Sun), sorted newest first
function buildWeeks(salaryEntries, penalties) {
  const weeksMap = {};

  const ensureWeek = (key) => {
    if (!weeksMap[key]) {
      weeksMap[key] = { key, entries: [], penalties: [] };
    }
    return weeksMap[key];
  };
  ...
  salaryEntries.forEach((e) => {
    ...
    ensureWeek(key).entries.push(e);
  });

  penalties.forEach((p) => {
    const key = weekKey(p.date);
    ensureWeek(key).penalties.push(p);
  });

  const weeks = Object.values(weeksMap).map((w) => {
    const days = w.entries.length;
    const fixedTotal = days * FIXED_DAILY;
    const accruedTotal = days * ACCRUED_DAILY;
    const revenueTotal = w.entries.reduce((s, e) => s + (e.revenuePercent || 0), 0);
    const penaltyTotal = w.penalties.reduce((s, p) => s + (p.amount || 0), 0);
    const payout = accruedTotal + revenueTotal - penaltyTotal;
    const grandTotal = fixedTotal + accruedTotal + revenueTotal - penaltyTotal;
    return {
      ...w,
      isCurrentWeek: w.key === weekKey(todayStr()),
      fixedTotal,
      accruedTotal,
      revenueTotal,
      penaltyTotal,
...
export default function App() {
  ...
  useEffect(() => {
    let cancelled = false;
    loadData(emptyData).then((loaded) => {
      if (!cancelled) {
        setData(loaded);
        setReady(true);
      }
  ...
  const save = useCallback((next) => {
    setData(next);
    setSyncing(true);
    saveData(next).then((ok) => {
      setSyncing(false);
      if (!ok) {
        setToast('Не удалось сохранить в облако. Проверь интернет.');
  ...
  if (!ready) {
    return (
      <div style={styles.app}>
        <div style={styles.loadingScreen}>Загрузка данных…</div>
      </div>
    );
  }
    ...
    <div style={styles.app}>
      <header style={styles.header}>
        <div style={styles.headerTitle}>
          Финансы
          {syncing && <span style={styles.syncDot} title="Сохранение…" />}
        </div>
...
function SummaryStrip({ data }) {
  const weeks = buildWeeks(data.salaryEntries, data.penalties);
  const currentWeek = weeks.find((w) => w.isCurrentWeek);
  const weekPayout = currentWeek ? currentWeek.payout : 0;
...
function SalaryTab({ data, save }) {
  ...
  const [penaltyDate, setPenaltyDate] = useState(todayStr());
  ...
  const addPenalty = () => {
    ...
    const p = { id: uid(), date: penaltyDate, amount: parseFloat(penaltyAmount) || 0, reason: penaltyReason.trim() };
    ...
    <div>
      ...
      <Section title="Штраф">
        {!penaltyMode ? (
          ...
          <div style={styles.form}>
            <label style={styles.label}>Дата штрафа (определяет, из какой недели вычтется)</label>
            <input style={styles.input} type="date" value={penaltyDate} onChange={(e) => setPenaltyDate(e.target.value)} />
        ...
        {weeks.length === 0 ? (
          ...
          weeks.map((w) => (
            <WeekCard
              key={w.key}
              week={w}
              isOpen={openWeek === w.key}
              onToggle={() => setOpenWeek(openWeek === w.key ? null : w.key)}
...
function WeekCard({ week, isOpen, onToggle, onRemoveEntry, onRemovePenalty }) {
  const sortedEntries = [...week.entries].sort((a, b) => b.date.localeCompare(a.date));
  const sortedPenalties = [...week.penalties].sort((a, b) => b.date.localeCompare(a.date));

  return (
    <div style={styles.weekCard}>
      <button style={styles.weekHeader} onClick={onToggle}>
        <div style={{ textAlign: 'left' }}>
          <div style={styles.weekLabel}>
            {weekRangeLabel(week.key)}
            {week.isCurrentWeek && <span style={styles.weekBadge}>текущая</span>}
          </div>
          <div style={styles.listItemSub}>{week.entries.length} {pluralDays(week.entries.length)}{week.penaltyTotal > 0 ? ` · штрафов на ${fmt(week.penaltyTotal)} ₽` : ''}</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={styles.weekTotal}>{fmt(week.grandTotal)} ₽</div>
          <div style={styles.listItemSub}>к выплате в конце: {fmt(week.payout)} ₽</div>
        </div>
      </button>

      {isOpen && (
        <div style={styles.weekBody}>
          <SummaryRow label="Фикс" value={`${fmt(week.fixedTotal)} ₽`} />
          <SummaryRow label="Накопительно" value={`${fmt(week.accruedTotal)} ₽`} />
          <SummaryRow label="5% от выручки" value={`${fmt(week.revenueTotal)} ₽`} />
          <SummaryRow label="Штрафы" value={`−${fmt(week.penaltyTotal)} ₽`} negative={week.penaltyTotal > 0} />
          <div style={styles.divider} />
          <SummaryRow label="Итого за неделю" value={`${fmt(week.grandTotal)} ₽`} bold />

          {sortedEntries.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <div style={styles.weekSubTitle}>Дни</div>
              {sortedEntries.map((e) => (
                <div key={e.id} style={styles.listItem}>
                  <div>
                    <div style={styles.listItemTitle}>{formatDate(e.date)}</div>
                    <div style={styles.listItemSub}>
                      {fmt(FIXED_DAILY)} ₽ + {fmt(ACCRUED_DAILY)} ₽ + {fmt(e.revenuePercent)} ₽ (5%)
                      {e.note && ` · ${e.note}`}
                    </div>
                  </div>
                  <button style={styles.deleteBtn} onClick={() => onRemoveEntry(e.id)}>
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
          ...
          {sortedPenalties.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <div style={styles.weekSubTitle}>Штрафы</div>
              {sortedPenalties.map((p) => (
                <div key={p.id} style={styles.listItem}>
                  ...
                  <button style={styles.deleteBtn} onClick={() => onRemovePenalty(p.id)}>
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
...
const styles = {
  ...
  loadingScreen: { display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', color: '#8A8681', fontSize: 14 },
  syncDot: { display: 'inline-block', width: 6, height: 6, borderRadius: '50%', background: '#D4A24C', marginLeft: 8, verticalAlign: 'middle' },
  ...
  weekCard: { background: '#242320', borderRadius: 14, border: '1px solid #302E2A', marginBottom: 10, overflow: 'hidden' },
  weekHeader: {
    width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center',
    padding: '14px 16px', background: 'transparent', border: 'none', cursor: 'pointer', color: '#F2EEE7',
  },
  weekLabel: { fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 },
  weekBadge: { fontSize: 10, fontWeight: 600, color: '#2F9E6E', background: 'rgba(47,158,110,0.15)', padding: '2px 6px', borderRadius: 6 },
  weekTotal: { fontFamily: "'JetBrains Mono', monospace", fontSize: 16, fontWeight: 600, color: '#F2EEE7' },
  weekBody: { padding: '0 16px 14px', borderTop: '1px solid #302E2A' },
  weekSubTitle: { fontSize: 11, fontWeight: 600, color: '#8A8681', marginTop: 10, marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.03em' },
